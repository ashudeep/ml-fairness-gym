...
def accumulate_rewards(rewards, gamma):
    ...
    acc = list(itertools.accumulate(
        reversed_rewards, lambda x, y: x*gamma + y))
...
def format_data(data_history, gamma, constant_baseline=0.0):
    ...
    inp_rec_seq, inp_rating_seq, output_recs, reward_weights = [], [], [], []
    for curr_recs, curr_rewards, curr_ratings in zip(data_history['recommendation_seqs'],
                                                     data_history['reward_seqs'], data_history['rating_seqs']):
        inp_rec_seq.append(curr_recs[:-1])
        # inp_reward_seq.append(curr_rewards[:-1])
        inp_rating_seq.append(curr_ratings[:-1])
    ...
    return {'input': [np.array(inp_rec_seq), np.array(inp_rating_seq)],
...
def format_data_safe_rl(data_history, gamma, constant_baseline=0.0):
    ...
    inp_rec_seq, inp_rating_seq, output_recs, reward_weights = [], [], [], []
    trajectories_cost = []
    for curr_recs, curr_ratings, curr_rewards, curr_safety_costs in zip(
            data_history['recommendation_seqs'],
            data_history['rating_seqs'],
            data_history['reward_seqs'],
            data_history['safety_costs']):
        inp_rec_seq.append(np.array(curr_recs[:-1]))
        inp_rating_seq.append(np.array(curr_ratings[:-1]))
...
def format_data_movielens(data_history, gamma, constant_baseline=0.0,
    ...
    if mask_already_recommended:
        ...
        masks_for_softmax = []
    for user_id, curr_recs, curr_rewards, curr_ratings, curr_safety_costs in zip(
...
def format_data_batch_movielens(data_history,
    ...
    for user_id, curr_recs, curr_rewards, curr_ratings, curr_safety_costs in zip(
            ...
            data_history['rewards'], data_history['ratings'], data_history['safety_costs']):
        inp_rec_seq.append(np.array(curr_recs[:-1]))
        inp_rating_seq.append(np.array(curr_ratings[:-1]))
        output_recs.append(np.expand_dims(np.array(curr_recs[1:]), axis=-1))
        output_rewards = accumulate_rewards(curr_rewards[1:] - constant_baseline,
